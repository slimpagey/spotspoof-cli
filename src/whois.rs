...
mod tests {
	...
	fn start_server(response: Vec<u8>, expected_method: &str, expected_path_prefix: &str) -> String {
	...
	fn check_domain_registration_parses_success() {
		let body = r#"{"Answer":[{"name":"example.com","type":2,"TTL":300,"data":"ns1.example.com."}]}"#;
		let response = format!(
			"HTTP/1.1 200 OK\r\nContent-Length: {}\r\n\r\n{}",
			body.len(),
			body
