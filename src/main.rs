...
struct Cli {
	#[command(subcommand)]
	command: Commands,
	#[arg(long, value_enum, default_value = "plain", help = "Log output format (plain or json)")]
...
enum Commands {
	...
	Import {
		...
		#[arg(
			long,
			default_value = "https://github.com/slimpagey/spotspoof-cli/releases/latest/spotspoof.sqlite.zst",
			help = "URL for a zstd-compressed SQLite database"
		)]
		url: String,
	},
...
async fn run(cli: Cli) -> Result<()> {
	match cli.command {
		...
		Commands::Import {
			...
			let db = resolve_db_path(db);
			if download {
				info!(kind = "import", action = "download", db_path = %db, url = %url);
...
fn format_ascii_text(results: &[crate::types::AsciiResult]) -> String {
	...
	results
		.iter()
		.map(|result| format!("Domain: {}, Similarity: {}", result.domain, result.similarity))
		.collect::<Vec<_>>()
		.join("; ")
}
...
fn format_ascii_csv(results: &[crate::types::AsciiResult]) -> Result<String> {
	...
	Ok(String::from_utf8(data).unwrap_or_default().trim_end().to_string())
}
